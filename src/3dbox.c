...
if(orib2[0][0][0]) {
// look for spin and diagonal reflect
// these only become meaningful if the dimensions indicate a square base or a cube
if(o->rng_x == o->rng_y) {
for(x=0; x<o->rng_x; ++x)
for(y=0; y<o->rng_y; ++y)
for(z=0; z<o->rng_z; ++z)
if(orib2[x][y][z] != orib2[y][x][z]) goto past_dr;
o->zero_dr = 1;
past_dr: ;
if(o->rng_x == o->rng_z) {
for(x=0; x<o->rng_x; ++x)
...
for(j=0; j<o_max; ++j) {
o = o_list + j;
if(dim_x != dim_y) o->zero_dr = 0;
if(dim_x != dim_y || dim_x != dim_z) o->zero_spin = 0;
}
if(!qtySpec) {
printf("warning: -%c option should be used with 1 instance of each piece; equivalent solutions will be generated\n", opt);
}else {
for(j=0; j<setSize; ++j)
if(qty[j] > 1 || qtyc[j] > 0) {
printf("warning: -%c option should be used with 1 instance of each piece; equivalent solutions will be generated\n", opt);
